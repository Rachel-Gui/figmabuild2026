...
export const ChatView = () => {
  return (
    <motion.div
      ...
      className="grid gap-4 lg:grid-cols-[minmax(0,1.2fr)_360px]"
    >
      <section className="app-surface relative overflow-hidden rounded-[32px] p-6 sm:p-8">
        ...
        <div className="mt-8 rounded-[28px] bg-[#e5e0d7] p-3">
          <div className="flex items-center gap-3 rounded-[22px] bg-[#f3efe8] px-4 py-4 text-sm text-[#7a7063]">
          ...
          {folders.map((folder) => (
            <div key={folder.title} className={`rounded-[28px] p-5 shadow-[0_14px_32px_rgba(49,50,56,0.06)] transition hover:-translate-y-0.5 hover:shadow-[0_18px_36px_rgba(49,50,56,0.08)] ${folder.tone}`}>
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="rounded-full bg-white/65 px-3 py-1 text-xs font-bold uppercase tracking-[0.18em] text-[#7a7063]">
                  {folder.count}
                </div>
              </div>

              <h3 className="mt-5 text-xl font-semibold text-[#313238]">{folder.title}</h3>
              <p className="mt-2 text-sm leading-6 text-[#7a7063]">{folder.updated}</p>
            </div>
          ))}
        </div>
              ...
              <div key={record.title} className="app-panel rounded-[24px] p-4">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <div className="mt-1 text-sm font-semibold text-[#7a7063]">{record.folder}</div>
                  </div>
                  <div className="inline-flex items-center gap-2 rounded-full bg-[#f3efe8] px-3 py-1 text-xs font-bold uppercase tracking-[0.18em] text-[#7a7063]">
                    <Clock3 size={12} />
                    {record.time}
                  </div>
                </div>
                <p className="mt-3 text-sm leading-6 text-[#7a7063]">{record.note}</p>
              </div>
            ))}
          </div>
        </div>
